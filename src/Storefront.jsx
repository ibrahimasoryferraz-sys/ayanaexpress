...
import {
  ...
  Truck, MessageCircle, CheckCircle2, ArrowLeft, Filter, SlidersHorizontal
} from "lucide-react";
...
const TESTIMONIALS = [
  { name: "Cátia M.", city: "Talatona", text: "Encomendei um perfume e chegou no mesmo dia. Atendimento muito atencioso pelo WhatsApp." },
  { name: "Nelson S.", city: "Kilamba", text: "Site fácil de usar e preços justos. Já fiz três compras e todas chegaram certinhas." },
  { name: "Ivana P.", city: "Viana", text: "Gostei da opção de pagar na entrega. Deu muita confiança para a primeira compra." },
];
  ...
  return (
    <header className="sticky top-0 z-40 bg-white border-b" style={{ borderColor: BRAND.line }}>
      <div className="max-w-6xl mx-auto px-4 py-3 flex items-center gap-3">
        ...
        <div className="flex items-center gap-1 cursor-pointer" onClick={() => onNav("home")}>
          <span className="text-xl font-black tracking-tight" style={{ color: BRAND.black }}>Ayana<span style={{ color: BRAND.red }}>Express</span></span>
        </div>
        ...
        <div className="fixed inset-0 z-50 bg-black/40" onClick={() => setMenuOpen(false)}>
          <div className="bg-white w-72 h-full p-5" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-6">
              <span className="text-lg font-black">Ayana<span style={{ color: BRAND.red }}>Express</span></span>
    ...
    <footer className="mt-16 text-white" style={{ backgroundColor: BRAND.black }}>
      ...
      <div className="max-w-6xl mx-auto px-4 py-10 grid grid-cols-2 md:grid-cols-4 gap-8 text-sm">
        ...
        <div>
          ...
          <div className="flex flex-col gap-1.5 text-white/60">
            <button className="text-left hover:text-white">Métodos de pagamento</button>
            <button className="text-left hover:text-white">Política de privacidade</button>
            <button className="text-left hover:text-white">Termos e condições</button>
            <button className="text-left hover:text-white">Política de devolução</button>
          </div>
        </div>
        <div>
          <p className="font-semibold mb-2">Contacto</p>
          <p className="text-white/60">WhatsApp: +244 976 340 762</p>
          <p className="text-white/60 mt-1">Luanda, Angola</p>
        </div>
      </div>
          ...
          <div className="grid sm:grid-cols-3 gap-4">
            {TESTIMONIALS.map((t, i) => (
              <div key={i} className="rounded-2xl bg-white p-5 border" style={{ borderColor: BRAND.line }}>
                ...
                <p className="text-xs font-semibold mt-3" style={{ color: BRAND.ink }}>{t.name} — {t.city}</p>
              </div>
            ))}
          </div>
        </div>
  ...
  if (submitted) {
    return (
      <div className="max-w-lg mx-auto px-4 py-16 text-center">
        ...
        <p className="text-sm mb-6" style={{ color: "#5C5852" }}>Finalize agora pelo WhatsApp para confirmarmos o seu pedido o mais rápido possível.</p>
        <a href={`https://wa.me/${WHATSAPP_NUMBER}?text=${encodeURIComponent(buildWhatsAppMessage())}`} target="_blank" rel="noopener noreferrer"
          className="inline-flex items-center gap-2 px-6 py-3 rounded-xl font-semibold text-sm text-white" style={{ backgroundColor: "#22C55E" }}>
...
export default function App() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const [prod, cats, zones] = await Promise.all([
